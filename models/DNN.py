...
class TCLS(nn.Module):
    """
    A deep neural network for the reverse diffusion process.
    """
    def __init__(self, in_dims, steps, norm=False, dropout=0.5):
        super(TCLS, self).__init__()
        self.in_dims = in_dims
        self.norm = norm
        self.d_cls = in_dims[-1]

        in_dims_temp = self.in_dims
        
        self.in_layers = nn.ModuleList([nn.Linear(d_in, d_out) \
            for d_in, d_out in zip(in_dims_temp[:-1], in_dims_temp[1:])])
        self.out_layer = nn.Linear(self.d_cls, steps)
    ...
    def init_weights(self):
        ...
        # Xavier Initialization for weights
        size = self.out_layer.weight.size()
        fan_out = size[0]
        fan_in = size[1]
        std = np.sqrt(2.0 / (fan_in + fan_out))
        self.out_layer.weight.data.normal_(0.0, std)

        # Normal Initialization for weights
        self.out_layer.bias.data.normal_(0.0, 0.001)
