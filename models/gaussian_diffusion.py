...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, model, x_start, steps, sampling_noise=False):
        ...
        ###p_sampling with langevin dynamics
    def p_sample_langevin(self, model, x_start, steps, sampling_noise=False, step_size=1e-7, exploration_length=1, exploration_number=1):
        assert steps <= self.steps, "Too much steps in inference."
        t = th.tensor([steps - 1] * x_start.shape[0]).to(x_start.device)
        x_t = self.q_sample(x_start, t)
        ...
        exploration_samples = []
        exploration_samples.append(x_t)
        for it in range(exploration_number):
            x_t = x_t.clone()
            # for j in range(2):
            #     x_t = self.langevin_step(model, x_t, t, 1e-2)
            #     # print("x_t shape: ",x_t.shape)
            for j in range(exploration_length):
                x_t = self.langevin_step(model, x_t, t, step_size)
                # print("x_t shape: ",x_t.shape)
            exploration_samples.append(x_t)
        
        # print("langevin", time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))

        # print(len(exploration_samples))
        for it in range(len(exploration_samples)):
            x_t = exploration_samples[it]
            for idx, i in enumerate(indices):
                t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
                # print("t shape: ", t.shape)
                # out = self.p_mean_variance(model, x_t, t)
                x_t = model(x_t,t)
            exploration_samples[it] = x_t

        # print("batch_0", time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
        return exploration_samples

    ###p_sampling with langevin gibbs
    def p_sample_langevin_gibbs(self, model, x_start, steps, time_classifier, sampling_noise=False, step_size=1e-7, exploration_length=1, exploration_number=1):
        assert steps <= self.steps, "Too much steps in inference."
        t = th.tensor([steps - 1] * x_start.shape[0]).to(x_start.device)
        x_t = self.q_sample(x_start, t)
        ...
        for it in range(exploration_number):
            ...
            # x_t = self.langevin_step(model, x_t, t, 1e-2)
            for j in range(exploration_length):
                x_t, t = self.gibbs_step(model, x_t, t, step_size, time_classifier)
                # print("x_t shape: ",x_t.shape)
            exploration_samples.append(x_t)
    ...
    ###p_sampling with langevin gibbs
    def p_sample_langevin_gibbs_jump(self, model, x_start, steps, time_classifier, sampling_noise=False, step_size=1e-7, exploration_length=1, exploration_number=1):
        assert steps <= self.steps, "Too much steps in inference."
        t = th.tensor([steps - 1] * x_start.shape[0]).to(x_start.device)
        x_t = self.q_sample(x_start, t)
    ...
    def langevin_step(self, model, batch, ts, step_size):
        noise = th.randn_like(batch).to(batch.device)
        if self.mean_type == ModelMeanType.START_X:
            score = (self._extract_into_tensor(self.sqrt_alphas_cumprod, ts, batch.shape) * model(batch, ts)-batch)/(1.-self._extract_into_tensor(self.alphas_cumprod, ts, batch.shape))
            # print(score.dtype, time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
            # score = (self.sqrt_alphas_cumprod[ts].unsqueeze(1)* model(batch, ts) - batch) /(1. - self.alphas_cumprod[ts].unsqueeze(1))
            updated_batch = batch + step_size*score/2 + math.sqrt(step_size) * noise
        # elif self.mean_type == ModelMeanType.EPSILON:
        #     score = -model(batch, ts)/ self.sqrt_one_minus_alphas_cumprod[ts].unsqueeze(1)
        #     updated_batch = batch - step_size/2 *  score + math.sqrt(step_size) * noise
        return updated_batch
    
    ###Gibbs Sampling Equation.
    def gibbs_step(self, model, batch, ts, step_size, time_classifier):
        ############################
        ### langevin step + most probable time alignment
        updated_batch = self.langevin_step(model, batch, ts, step_size)
        updated_time = th.argmax(time_classifier(updated_batch), dim=1)
        return updated_batch, updated_time
        ############################
