...
#------------------------------------------------------------
# more exploration of L-DiffRec
parser.add_argument('--exploration_length', type=int, default=5, help='exploration_length')
parser.add_argument('--exploration_number', type=int, default=5, help='exploration_number')
parser.add_argument('--exploration_size', type=float, default=1e-7, help='step size of the langevin dynamics')
...
model = torch.load(model_path + args.dataset + model_name).to(device)
Autoencoder = torch.load(model_path + args.dataset + AE_name).to(device)

##-------------------------------------------------
# # build time classifier
# mlp_in_dims = [300,300]
# time_classifier = TCLS(mlp_in_dims, args.steps)
# time_classifier.load_state_dict(torch.load('./checkpoints/time_classifier_epoch_600_steps5_min0.0005_max0.1_latent_ver2.pth'))
...
def evaluate(data_loader, data_te, mask_his, topN):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader):
            ...
            prediction_list = diffusion.p_sample_langevin(model, batch_latent, args.sampling_steps, args.sampling_noise, args.exploration_size, args.exploration_length, args.exploration_number)
            batch_latent_recon = torch.sum(torch.stack(prediction_list), dim=0)
